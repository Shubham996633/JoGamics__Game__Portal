...
const ghostEat = document.querySelector('.eat-ghost')
const pacdeath = document.querySelector('.pacman-death')
...
const pacmanPower = document.querySelector('.pacman-power')
const pacmanWin = document.querySelector('.pacman-win')
...
function circleCollidesWithRectangle({
    ...
    return ( circle.position.y - circle.radius + circle.velocity.y <= rectangle.position.y + rectangle.height + padding && circle.position.x + 
        circle.radius + circle.velocity.x >= rectangle.position.x - padding && 
        circle.position.y + circle.radius + circle.velocity.y >= rectangle.position.y - padding && circle.position.x - 
        circle.radius + circle.velocity.x <= rectangle.position.x + rectangle.width + padding)
}
    ...
    for(let i = pellets.length - 1; 0 <= i; i--){
        ...
        if(Math.hypot(pellet.position.x - player.position.x,
            ...
            player.radius){
                ...
                document.querySelector('.game-score').innerHTML = `Your Score is ${score-10}`
            }
